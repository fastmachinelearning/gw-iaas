...
@pytest.mark.torch
def test_torch_onnx_exporter(temp_repo, torch_model):
    ...
    version_path = temp_repo.fs.join("hermes-quiver-test", "identity", "1")
    temp_repo.fs.soft_makedirs(version_path)

    output_path = temp_repo.fs.join(version_path, "model.onnx")
    ...
    model2 = Model("identity2", temp_repo, Platform.ONNX)
